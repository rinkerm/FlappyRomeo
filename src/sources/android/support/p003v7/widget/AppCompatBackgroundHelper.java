...
class AppCompatBackgroundHelper {
    ...
    public void loadFromAttributes(AttributeSet attrs, int defStyleAttr) {
        ColorStateList tint;
        TypedArray a = this.mView.getContext().obtainStyledAttributes(attrs, C0232R.styleable.ViewBackgroundHelper, defStyleAttr, 0);
        try {
            if (a.hasValue(C0232R.styleable.ViewBackgroundHelper_android_background) && (tint = this.mTintManager.getTintList(a.getResourceId(C0232R.styleable.ViewBackgroundHelper_android_background, -1))) != null) {
                setInternalBackgroundTint(tint);
            }
            if (a.hasValue(C0232R.styleable.ViewBackgroundHelper_backgroundTint)) {
                ViewCompat.setBackgroundTintList(this.mView, a.getColorStateList(C0232R.styleable.ViewBackgroundHelper_backgroundTint));
            }
            if (a.hasValue(C0232R.styleable.ViewBackgroundHelper_backgroundTintMode)) {
                ViewCompat.setBackgroundTintMode(this.mView, DrawableUtils.parseTintMode(a.getInt(C0232R.styleable.ViewBackgroundHelper_backgroundTintMode, -1), (PorterDuff.Mode) null));
            }
        } finally {
            a.recycle();
        }
